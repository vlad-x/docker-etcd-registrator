    ...
    stream
      .on('data', function(buff) {
        ...
        switch(obj.status) {
          ...
          case 'pause':
          case 'unpause':
            if(self.startDebounce['sync']) {
              clearTimeout(self.startDebounce['sync']);
              delete self.startDebounce['sync'];
            }
            self.startDebounce['sync'] = setTimeout(function() {
              delete self.startDebounce['sync'];
              self.emit('sync');
            }, self.opts.startDebounce);
            break;
...
Docker.prototype._parseInspect = function parseInspect(obj) {
  var data = {
    ...
    ip: obj.NetworkSettings.IPAddress,
    ports: [],
    env: {},
